...
class ReturnsActivity : AppCompatActivity() {
    ...
    private fun form(kind:String){val r=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;layoutDirection=LinearLayout.LAYOUT_DIRECTION_RTL;setBackgroundColor(bg);setPadding(dp(14),dp(14),dp(14),dp(14));addView(tv(kind,23f,true))};val party=field(if(kind=="مرتجع بيع")"اسم العميل" else "اسم المورد");val date=field("التاريخ");val code=field("رمز الصنف");val qty=field("الكمية");val price=field("سعر المرتجع");val ref=field("المرجع");listOf(date,party,code,qty,price,ref).forEach{r.addView(it,LinearLayout.LayoutParams(-1,dp(48)).apply{setMargins(0,dp(3),0,dp(3))})};r.addView(tv("نوع السداد",14f,true));val payment=Spinner(this).apply{adapter=ArrayAdapter(this@ReturnsActivity,android.R.layout.simple_spinner_dropdown_item,arrayOf("نقدي","آجل"))};r.addView(payment,LinearLayout.LayoutParams(-1,dp(50)).apply{setMargins(0,dp(3),0,dp(6))});r.addView(tv("في الآجل يتم عكس الرصيد على حساب العميل/المورد",12f));r.addView(tv("الأصناف الحالية: ${db.items().joinToString("، ")}",12f));r.addView(btn("حفظ المرتجع"){val id=findH2proItemId(code.text.toString().trim());val q=qty.text.toString().toDoubleOrNull()?:0.0;val p=price.text.toString().toDoubleOrNull()?:0.0;if(id<=0||q<=0||p<0||party.text.isBlank()){toast("تحقق من البيانات");return@btn};val mode=payment.selectedItem?.toString()?:"نقدي";val doc=db.saveReturn(kind,date.text.toString().ifBlank{"2026-08-26"},party.text.toString(),listOf(InvoiceLine(id,q,p)),ref.text.toString(),"",mode);toast(if(doc>0)"تم حفظ المرتجع وتحديث المخزون والقيد" else "تعذر حفظ المرتجع أو الرصيد غير كاف");if(doc>0)home()});r.addView(btn("رجوع"){home()});setContentView(ScrollView(this).apply{addView(r,ViewGroup.LayoutParams(-1,-1))})}
