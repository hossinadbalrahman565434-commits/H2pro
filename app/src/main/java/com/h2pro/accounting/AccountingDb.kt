...
class AccountingDb(context: Context) : SQLiteOpenHelper(context, "h2pro.db", null, 7) {
    ...
    private fun createTables(db: SQLiteDatabase) {
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS items(id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT UNIQUE, name TEXT, buy REAL DEFAULT 0, sale REAL DEFAULT 0, avg_cost REAL DEFAULT 0, qty REAL DEFAULT 0, min_qty REAL DEFAULT 0, unit TEXT DEFAULT 'قطعة')")
        ...
        addColumnIfMissing(db, "items", "avg_cost", "REAL DEFAULT 0")
        addColumnIfMissing(db, "invoice_lines", "cost_total", "REAL DEFAULT 0")
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS invoice_lines(id INTEGER PRIMARY KEY AUTOINCREMENT, document_id INTEGER, item_id INTEGER, qty REAL, price REAL, total REAL, cost_total REAL DEFAULT 0)")
        ...
        db.execSQL("CREATE INDEX IF NOT EXISTS idx_inventory_item_date ON inventory_movements(item_id,date,id)")")
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        createTables(db)
        migratePasswords(db)
        db.execSQL("UPDATE items SET avg_cost=buy WHERE avg_cost IS NULL OR avg_cost=0")
        db.execSQL("UPDATE invoice_lines SET cost_total=0 WHERE cost_total IS NULL")
        seed(db)
        ensureRequiredAccounts(db)
    }
    ...
    fun addItem(code: String, name: String, buy: Double, sale: Double, qty: Double, minQty: Double, unit: String = "قطعة") = try { insert("items", ContentValues().apply { put("code", code); put("name", name); put("buy", buy); put("sale", sale); put("avg_cost", buy); put("qty", qty); put("min_qty", minQty); put("unit", unit) }) > 0 } catch (_: Exception) { false }
    ...
    fun sumDocuments(kind: String): Double = readableDatabase.rawQuery("SELECT COALESCE(SUM(amount),0) FROM documents WHERE kind=? AND substr(date,1,4)=?", arrayOf(kind, sessionYear.toString())).use { if (it.moveToFirst()) it.getDouble(0) else 0.0 }
    fun inventoryValue(): Double = readableDatabase.rawQuery("SELECT COALESCE(SUM(qty*COALESCE(avg_cost,buy)),0) FROM items", null).use { if (it.moveToFirst()) it.getDouble(0) else 0.0 }
    ...
    fun saveInvoice(kind: String, date: String, party: String, lines: List<InvoiceLine>, reference: String = "", notes: String = "", paymentMode: String = "نقدي"): Long {
        ...
        if (paymentMode != "نقدي" && paymentMode != "آجل") return -1
        if (lines.isEmpty() || lines.any { it.itemId <= 0 || it.qty <= 0 || it.price < 0 }) return -1
        val total = lines.sumOf { it.qty * it.price }
        if (total <= 0) return -1

        val sale = kind == "بيع"
        val credit = paymentMode == "آجل"
        val d = writableDatabase
        d.beginTransaction()
        return try {
            val prepared = lines.map { line ->
                d.rawQuery("SELECT qty,COALESCE(avg_cost,buy),buy FROM items WHERE id=?", arrayOf(line.itemId.toString())).use { c ->
                    if (!c.moveToFirst()) throw IllegalArgumentException("الصنف غير موجود")
                    val qty = c.getDouble(0)
                    val avg = c.getDouble(1)
                    Triple(line, qty, avg)
                }
            }
            if (sale && prepared.any { it.second < it.first.qty }) throw IllegalArgumentException("الرصيد غير كاف")

            val doc = d.insertOrThrow("documents", null, ContentValues().apply {
                put("kind", kind); put("date", date); put("name", party); put("amount", total)
                put("reference", reference); put("notes", notes); put("payment_mode", paymentMode)
            })

            var totalCost = 0.0
            prepared.forEach { (line, oldQty, oldAvg) ->
                val lineCost = if (sale) oldAvg * line.qty else oldAvg * line.qty
                totalCost += lineCost
                d.insertOrThrow("invoice_lines", null, ContentValues().apply {
                    put("document_id", doc); put("item_id", line.itemId); put("qty", line.qty)
                    put("price", line.price); put("total", line.qty * line.price); put("cost_total", lineCost)
                })

                if (sale) {
                    d.execSQL("UPDATE items SET qty=qty-?, sale=? WHERE id=?", arrayOf(line.qty, line.price, line.itemId.toString()))
                } else {
                    val newQty = oldQty + line.qty
                    val newAvg = if (newQty > 0) ((oldQty * oldAvg) + (line.qty * line.price)) / newQty else line.price
                    d.execSQL("UPDATE items SET qty=?, buy=?, avg_cost=? WHERE id=?", arrayOf(newQty, line.price, newAvg, line.itemId.toString()))
                }
                saveInventoryMovementWithDb(d, line.itemId, date, kind, line.qty, if (sale) oldAvg else line.price, reference)
            }

            val cash = findAccountId(d, "101")
            val inventory = findAccountId(d, "103")
            val revenue = findAccountId(d, "401")
            val ar = findAccountId(d, "202")
            val ap = findAccountId(d, "201")
            val cogs = findAccountId(d, "503")
            require(inventory > 0)

            val jid = d.insertOrThrow("journals", null, ContentValues().apply {
                put("date", date)
                put("description", if (sale) "فاتورة بيع: $party" else "فاتورة شراء: $party")
                put("reference", reference)
            })
            if (sale) {
                val debitAccount = if (credit) ar else cash
                require(debitAccount > 0 && revenue > 0 && cogs > 0)
                addLine(d, jid, debitAccount, total, 0.0)
                addLine(d, jid, revenue, 0.0, total)
                addLine(d, jid, cogs, totalCost, 0.0)
                addLine(d, jid, inventory, 0.0, totalCost)
            } else {
                val creditAccount = if (credit) ap else cash
                require(creditAccount > 0)
                addLine(d, jid, inventory, total, 0.0)
                addLine(d, jid, creditAccount, 0.0, total)
            }
    ...
    fun saveReturn(kind: String, date: String, party: String, lines: List<InvoiceLine>, reference: String = "", notes: String = "", paymentMode: String = "نقدي"): Long {
        ...
        val purchaseReturn = kind == "مرتجع شراء"
        val credit = paymentMode == "آجل"
        val d = writableDatabase
        d.beginTransaction()
        return try {
            val prepared = lines.map { line ->
                d.rawQuery("SELECT qty,COALESCE(avg_cost,buy) FROM items WHERE id=?", arrayOf(line.itemId.toString())).use { c ->
                    if (!c.moveToFirst()) throw IllegalArgumentException("الصنف غير موجود")
                    Pair(line, doubleArrayOf(c.getDouble(0), c.getDouble(1)))
                }
            }
            if (purchaseReturn && prepared.any { it.second[0] < it.first.qty }) throw IllegalArgumentException("الرصيد غير كاف للمرتجع")

            val total = lines.sumOf { it.qty * it.price }
            if (total <= 0) throw IllegalArgumentException("قيمة غير صحيحة")
            val doc = d.insertOrThrow("documents", null, ContentValues().apply {
                put("kind", kind); put("date", date); put("name", party); put("amount", total)
                put("reference", reference); put("notes", notes); put("payment_mode", paymentMode)
            })
            ...
            var totalCost = 0.0
            prepared.forEach { (line, state) ->
                val oldQty = state[0]
                val avg = state[1]
                val lineCost = avg * line.qty
                totalCost += lineCost
                d.insertOrThrow("invoice_lines", null, ContentValues().apply {
                    put("document_id", doc); put("item_id", line.itemId); put("qty", line.qty)
                    put("price", line.price); put("total", line.qty * line.price); put("cost_total", lineCost)
                ...
                if (purchaseReturn) {
                    val newQty = oldQty - line.qty
                    d.execSQL("UPDATE items SET qty=? WHERE id=?", arrayOf(newQty, line.itemId.toString()))
                } else {
                    val newQty = oldQty + line.qty
                    d.execSQL("UPDATE items SET qty=? WHERE id=?", arrayOf(newQty, line.itemId.toString()))
                }
                saveInventoryMovementWithDb(d, line.itemId, date, kind, line.qty, avg, reference)
            }

            val cash = findAccountId(d, "101")
            val inventory = findAccountId(d, "103")
            val salesReturn = findAccountId(d, "402")
            val ar = findAccountId(d, "202")
            val ap = findAccountId(d, "201")
            val cogs = findAccountId(d, "503")
            val jid = d.insertOrThrow("journals", null, ContentValues().apply {
                put("date", date); put("description", "$kind: $party"); put("reference", reference)
            })

            if (purchaseReturn) {
                val debit = if (credit) ap else cash
                require(debit > 0 && inventory > 0)
                addLine(d, jid, debit, total, 0.0)
                addLine(d, jid, inventory, 0.0, totalCost)
                val diff = total - totalCost
                if (kotlin.math.abs(diff) > 0.005) {
                    val purchases = findAccountId(d, "501")
                    require(purchases > 0)
                    if (diff > 0) addLine(d, jid, purchases, diff, 0.0)
                    else addLine(d, jid, purchases, 0.0, -diff)
                }
            } else {
                val creditAccount = if (credit) ar else cash
                require(salesReturn > 0 && creditAccount > 0 && inventory > 0 && cogs > 0)
                addLine(d, jid, salesReturn, total, 0.0)
                addLine(d, jid, creditAccount, 0.0, total)
                addLine(d, jid, inventory, totalCost, 0.0)
                addLine(d, jid, cogs, 0.0, totalCost)
            }
            d.setTransactionSuccessful()
            doc
        } catch (_: Exception) {
    ...
    fun accountBalance(id: Long): Double = readableDatabase.rawQuery("SELECT COALESCE(SUM(jl.debit-jl.credit),0) FROM journal_lines jl JOIN journals j ON j.id=jl.journal_id WHERE jl.account_id=? AND substr(j.date,1,4)=?", arrayOf(id.toString(), sessionYear.toString())).use { if (it.moveToFirst()) it.getDouble(0) else 0.0 }
    ...
    fun trialBalanceTotals(): TrialBalanceTotals { val row = readableDatabase.rawQuery("SELECT COALESCE(SUM(jl.debit),0),COALESCE(SUM(jl.credit),0) FROM journal_lines jl JOIN journals j ON j.id=jl.journal_id WHERE substr(j.date,1,4)=?", arrayOf(sessionYear.toString())).use { if (it.moveToFirst()) doubleArrayOf(it.getDouble(0), it.getDouble(1)) else doubleArrayOf(0.0, 0.0) }; return TrialBalanceTotals(row[0], row[1], abs(row[0] - row[1]) < 0.005) }
