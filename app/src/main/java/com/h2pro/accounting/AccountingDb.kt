...
class AccountingDb(context: Context) : SQLiteOpenHelper(context, "h2pro.db", null, 6) {
    ...
    fun hasPermission(userNo: String, module: String): Boolean { val role=readableDatabase.rawQuery("SELECT role FROM users WHERE user_no=? AND active=1 LIMIT 1",arrayOf(userNo)).use{if(it.moveToFirst())it.getString(0) else ""}; if(role=="مدير النظام") return true; return readableDatabase.rawQuery("SELECT allowed FROM permissions p JOIN users u ON u.id=p.user_id WHERE u.user_no=? AND p.module=? LIMIT 1",arrayOf(userNo,module)).use{it.moveToFirst()&&it.getInt(0)==1} }
