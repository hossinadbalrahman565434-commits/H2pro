...
class SalesActivity : AppCompatActivity() {
    ...
    private fun invoice(){val r=root("فاتورة بيع جديدة");val date=field("التاريخ");val customer=field("اسم العميل");val code=field("رمز الصنف");val qty=field("الكمية");val price=field("سعر البيع");val ref=field("رقم الفاتورة / المرجع");val notes=field("البيان");listOf(date,customer,code,qty,price,ref,notes).forEach{r.addView(it,LinearLayout.LayoutParams(-1,dp(50)).apply{setMargins(0,dp(3),0,dp(3))})};r.addView(tv("نوع السداد",14f,true));val payment=Spinner(this).apply{adapter=ArrayAdapter(this@SalesActivity,android.R.layout.simple_spinner_dropdown_item,arrayOf("نقدي","آجل"))};r.addView(payment,LinearLayout.LayoutParams(-1,dp(50)).apply{setMargins(0,dp(3),0,dp(6))});r.addView(tv("الآجل يسجل على حساب العملاء",12f));r.addView(tv("الأصناف المتاحة: ${db.items().joinToString("، ")}",13f));r.addView(btn("حفظ فاتورة البيع"){val itemId=findH2proItemId(code.text.toString().trim());val q=qty.text.toString().toDoubleOrNull()?:0.0;val p=price.text.toString().toDoubleOrNull()?:0.0;if(itemId<=0||q<=0||p<0||customer.text.isBlank()){toast("تحقق من العميل والصنف والكمية والسعر");return@btn};val mode=payment.selectedItem?.toString()?:"نقدي";val id=db.saveInvoice("بيع",date.text.toString().ifBlank{"2026-08-26"},customer.text.toString(),listOf(InvoiceLine(itemId,q,p)),ref.text.toString(),notes.text.toString(),mode);toast(if(id>0)"تم حفظ الفاتورة وتحديث المخزون والقيد" else "تعذر حفظ الفاتورة أو الرصيد غير كاف");if(id>0)home()});r.addView(btn("رجوع"){home()});show(r)}
