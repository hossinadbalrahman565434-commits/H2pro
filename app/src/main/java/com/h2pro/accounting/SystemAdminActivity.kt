...
class SystemAdminActivity : AppCompatActivity() {
    ...
    private val bg=Color.rgb(12,28,40); private val surface=Color.rgb(24,45,60); private val gold=Color.rgb(198,161,91); private val text=Color.rgb(245,241,232)
    private fun dp(v:Int)= (v*resources.displayMetrics.density).toInt()
    private fun title(s:String)=TextView(this).apply{text=s;textSize=23f;setTextColor(text);gravity=Gravity.RIGHT}
    ...
    private fun field(hint:String)=EditText(this).apply{this.hint=hint;setTextColor(text);setHintTextColor(Color.LTGRAY);textSize=16f;gravity=Gravity.RIGHT;setPadding(dp(12),0,dp(12),0)}
    ...
    private fun users(){val r=root("بيانات المستخدمين");val no=field("رقم المستخدم");val name=field("اسم المستخدم");val pass=field("كلمة السر");val role=field("الدور / الصلاحية");val search=field("بحث");add(r,no);add(r,name);add(r,pass);add(r,role)
    ...
    private fun showText(r:LinearLayout,t:String,s:String){val v=TextView(this).apply{text="$t\n\n$s";setTextColor(text);textSize=15f;gravity=Gravity.RIGHT;setPadding(0,dp(8),0,dp(8))};r.addView(v,0,LinearLayout.LayoutParams(-1,-2))}
