...
class PurchaseActivity : AppCompatActivity() {
    ...
    private fun invoice(){val r=root("فاتورة شراء جديدة");val date=field("التاريخ");val supplier=field("اسم المورد");val code=field("رمز الصنف");val qty=field("الكمية");val price=field("سعر الشراء");val ref=field("رقم الفاتورة / المرجع");val notes=field("البيان");listOf(date,supplier,code,qty,price,ref,notes).forEach{r.addView(it,LinearLayout.LayoutParams(-1,dp(50)).apply{setMargins(0,dp(3),0,dp(3))})};r.addView(tv("الأصناف المتاحة: ${db.items().joinToString("، ")}",13f));r.addView(btn("حفظ فاتورة الشراء"){val itemId=db.findItemId(code.text.toString().trim());val q=qty.text.toString().toDoubleOrNull()?:0.0;val p=price.text.toString().toDoubleOrNull()?:0.0;if(itemId<=0||q<=0||p<0||supplier.text.isBlank()){toast("تحقق من المورد والصنف والكمية والسعر");return@btn};val id=db.saveInvoice("شراء",date.text.toString().ifBlank{"2026-08-26"},supplier.text.toString(),listOf(InvoiceLine(itemId,q,p)),ref.text.toString(),notes.text.toString());toast(if(id>0)"تم حفظ فاتورة الشراء وتحديث المخزون" else "تعذر حفظ الفاتورة");if(id>0)home()});r.addView(btn("رجوع"){home()});show(r)}
