...
class PurchaseActivity : AppCompatActivity() {
    ...
    private fun home(){val r=root("إدارة المشتريات");r.addView(tv("فواتير الشراء وتحديث المخزون تلقائيًا",16f,true));r.addView(btn("فاتورة شراء جديدة"){invoice()});r.addView(btn("الموردون"){suppliers()});r.addView(btn("تقرير المشتريات"){report()});r.addView(btn("رجوع"){finish()});show(r)}
    private fun invoice(){val r=root("فاتورة شراء جديدة");val date=field("التاريخ");val supplier=field("اسم المورد");val code=field("رمز الصنف");val qty=field("الكمية");val price=field("سعر الشراء");val ref=field("رقم الفاتورة / المرجع");val notes=field("البيان");listOf(date,supplier,code,qty,price,ref,notes).forEach{r.addView(it,LinearLayout.LayoutParams(-1,dp(50)).apply{setMargins(0,dp(3),0,dp(3))})};r.addView(tv("الأصناف المتاحة: ${db.items().joinToString("، ")}",13f));r.addView(btn("حفظ فاتورة الشراء"){val itemId=findH2proItemId(code.text.toString().trim());val q=qty.text.toString().toDoubleOrNull()?:0.0;val p=price.text.toString().toDoubleOrNull()?:0.0;if(itemId<=0||q<=0||p<0||supplier.text.isBlank()){toast("تحقق من المورد والصنف والكمية والسعر");return@btn};val id=db.saveInvoice("شراء",date.text.toString().ifBlank{"2026-08-26"},supplier.text.toString(),listOf(InvoiceLine(itemId,q,p)),ref.text.toString(),notes.text.toString());toast(if(id>0)"تم حفظ فاتورة الشراء وتحديث المخزون" else "تعذر حفظ الفاتورة");if(id>0)home()});r.addView(btn("رجوع"){home()});show(r)}
